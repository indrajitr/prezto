...
# ls
# Detect `ls` provided by GNU Core Utilities
gnuls=0
if grep -q -i 'GNU' <(ls --version 2>&1); then
  gnuls=1
fi

if (( $gnuls )) && zstyle -T ':prezto:module:utility:ls' dirs-first; then
...
if zstyle -t ':prezto:module:utility:ls' color; then
  ...
  if (( $gnuls )); then
    ...
    if [[ -z "$LS_COLORS" ]]; then
      ...
      # Try {,g}dircolors when available, preferring local colors if present
      if (( $#commands[(i)(|g)dircolors] )); then
        eval "$($commands[(i)(|g)dircolors] --sh $HOME/.dir_colors(.N))"
      else
...
alias sl='ls'            # Common spelling error.
